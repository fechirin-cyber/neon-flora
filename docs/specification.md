...
### 9.1 方式: clip_contents連続ストリップスクロール
...
### 9.2 スクロール物理（実機準拠 — 法規制/特許文献ベース）

| 項目 | 値 | 根拠 |
|---|---|---|
| 回転速度 | **80 RPM** (4592 px/sec) | 法規制上限: 別表第5 |
| 1コマ通過時間 | **35.7ms** | 0.75秒/周 ÷ 21コマ |
| 加速時間 | **0.4秒** | 特許JP2002159626A |
| 停止方式 | **コマ送り即停止** | ステッピングモーター全相励磁制動 |
| 停止時間 | **0〜143ms** (法規制190ms以内) | 0〜4コマ × 35.7ms |
| 最大滑り | **4コマ** | 190ms / 35.7ms ≒ 5.3 → 安全マージン |
| 図柄高さ+隙間 | 164px (160+4) | — |
| バウンス | 下方向1.5px → 戻り（20ms + 30ms = 50ms） | ステッピングモーター微振動 |
| STOPボタン有効化 | 全リールフル回転到達後 | 実機仕様 |
...
# STOPボタン押下: FULL_SPEED → 即座にスナップ停止 → BOUNCING → IDLE
# 滑り計算(0-4コマ)はSlotEngine/ReelLogicが担当、ReelStripは結果を即反映
```

### 9.3.1 モーションブラー（reel_blur.gdshader）

回転中の残像効果を縦方向モーションブラーで再現:
- `blur_strength`: 0.0（停止）〜 1.0（フル回転80RPM）
- 加速中は速度に比例して徐々にブラー強度が上がる
- 停止時は即座にblur_strength = 0（クリアな図柄表示）
- 4-tap weighted blur + 軽微な彩度低下（実機の残像感）
...
### 9.5 ドラム曲面シェーダー（reel_drum.gdshader）
...
### 9.6 ガラス反射シェーダー（reel_glass.gdshader）
